...
title: Java数组
toc: true
data: 2022-09-22 18:08:51
tags: Java笔记
categories: java
---

*Java 语言中提供的数组是用来存储固定大小的同类型元素。*<!--more-->
...
![1561452334825](Java数组/1561452334825.png)
...
- 一维数组：存储一组数据
- 二维数组：存储多组数据，相当于二维表，一行代表一组数据，这里的二维表每一行长度不要求一样。

![image-20211221164709624](Java数组/image-20211221164709624.png)
...
![image-20220922220247784](Java数组/image-20220922220247784.png)

### 7、数组元素的默认值

当我们使用动态初始化方式创建数组时，元素只是默认值。

![1561509460135](Java数组/1561509460135.png)
...
![1561465258546](Java数组/1561465258546.png)
...
![数组内存图1](Java数组/数组内存图1.jpg)

<img src="Java数组/image-20220923095252561.png" alt="image-20220923095252561" style="zoom:80%;" />
...
<img src="Java数组/image-20220923095546844.png" alt="image-20220923095546844" style="zoom:80%;" />
...
因为第一个元素距离数组首地址间隔0个单元格。（偏移量）

![image-20220923095912590](Java数组/image-20220923095912590.png)
...
一楼到十楼的每层电梯门口都放着一颗钻石，钻石大小不一。你乘坐电梯从一楼到十楼，每层楼电梯门都会打开一次，手里只能拿一颗钻石，问怎样才能拿到最大的一颗？
...
先把要比较的元素一分为二，然后把中间元素mid与目标元素比较，

如果相同，找到。

如果不同，缩小范围，

​					如果 目标 比中间元素大，下次查找元素在右边，把left = mid + 1

​					如果 目标 比中间元素小，下次查找元素在在边，把right = mid - 1

如果一直没找到，当left > right 结束。
...
public class Test15ArrayBinarySearch {
    public static void main(String[] args){
        ...
        // 可以加俩个else if分支 比较mid的同时 left和right 可以减少一次循环。
        if(index!=-1){
...
![image-20220922224217611](Java数组/image-20220922224217611.png)
...
![image-20220922224600171](Java数组/image-20220922224600171.png)
...
![image-20211222111142684](Java数组/image-20211222111142684.png)

- 时间复杂度：

常见的算法时间复杂度由小到大依次为：Ο(1)＜Ο(log~2~n)＜Ο(n)＜Ο(nlog~2~n)＜Ο(n<sup>2</sup>)＜Ο(n<sup>3</sup>)＜…＜Ο(2<sup>n</sup>)＜Ο(n!)

![image-20220923163354151](Java数组/image-20220923163354151.png)
...
![image-20211222113701365](Java数组/image-20211222113701365.png)
...
class Test19BubbleSort2{
	public static void main(String[] args){
		...
		for(int i=1; i<arr.length; i++){ 
			...
			for(int j=0; j<arr.length-i; j++){// 后面排好的不用再排了
				//count++;
				//希望的是arr[j] < arr[j+1]
				if(arr[j] > arr[j+1]){
					//交换arr[j]与arr[j+1]
